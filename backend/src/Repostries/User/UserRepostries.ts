...
export class UserRepository extends BaseRepository<typeof userModel> implements IUserRepository {
    ...
    async emailExistCheck(email: string): Promise<UserInterface | null> {
        try {
            ...
            const existingUser = await this.model.find({ email: email });
    ...
    async editUser(userId: string): Promise<UserInterface | null> {
        try {
            ...
            let check = await this.model.findById(userId);
    ...
    async updateUser(userData: UserInterface, id: string): Promise<UserInterface | null> {
        try {
            const editUser = await this.model.findByIdAndUpdate(
                { _id: id },
                {
    ...
    async countUsers(): Promise<number | null> {
        try {
            const countUsers = await this.model.countDocuments();
            return countUsers;
        } catch (error) {
            return null;
        }
